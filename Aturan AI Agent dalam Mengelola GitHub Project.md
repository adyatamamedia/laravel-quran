...
1. Memeriksa dan melakukan sinkronisasi dengan remote repository (`git fetch` & `git pull`) **SEBELUM mulai bekerja**, karena pengembangan dilakukan dari beberapa perangkat (PC & Laptop).
2. Memeriksa kondisi repository lokal sebelum bekerja.
3. Memahami perubahan yang sudah ada sebelum membuat perubahan baru.
4. Tidak menghapus pekerjaan developer atau agent lain.
5. Membuat perubahan secara bertahap dan terkontrol.
6. Melakukan validasi sebelum commit.
7. Menulis commit message yang menjelaskan perubahan.
8. Menghindari tindakan Git destruktif.
9. Menjaga credential dan data sensitif agar tidak masuk repository.

---

# 2. PEMERIKSAAN AWAL & SINKRONISASI REMOTE SEBELUM BEKERJA

Dikarenakan pengembangan aplikasi dilakukan secara fleksibel dari beberapa perangkat (seperti PC Desktop dan Laptop), **AI Agent WAJIB memastikan repository lokal selalu sinkron dengan GitHub SEBELUM mulai melakukan pekerjaan apapun**.

Sebelum mengubah atau menginspeksi source code untuk tugas baru, jalankan:

```bash
git fetch
```

Kemudian periksa status dan posisi commit terhadap remote:

```bash
git status
```

Jika branch lokal tertinggal dari remote (`Your branch is behind...`), AI Agent WAJIB langsung menarik perubahan terbaru terlebih dahulu:

```bash
git pull origin <nama-branch>
```

Selanjutnya periksa branch aktif dan histori commit:
...
- apakah local repository sudah diperbarui dari remote GitHub (perubahan dari PC/Laptop lain),
...
Jika terdapat perubahan yang dibuat developer dari perangkat lain (misal commit baru dari laptop), **jangan langsung menimpa atau mengabaikannya, tarik (`pull`) perubahan tersebut terlebih dahulu**.
...
Workflow default (WAJIB diawali sinkronisasi remote dari PC/Laptop):
...
git status
```
*(Jalankan `git pull` jika tertinggal dari remote)*
...
Jika diminta atau workflow mengharuskan (agar siap diakses dari perangkat PC/Laptop lain):
